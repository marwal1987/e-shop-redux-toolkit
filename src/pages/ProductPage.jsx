    ...
    <div className="container m-auto h-screen flex flex-col items-center justify-center">
      {product ? (
        <div className="flex gap-12 items-center justify-around shadow-lg p-12 border-b-2 border-gray-200">
          ...
          <button className="bg-[#f3929c] text-gray-100 p-2 rounded font-bold hover:bg-[#3a3f54] " onClick={() => dispatch(addToCart(product))}>Add to Cart</button>
          <button className="bg-[#3a3f54] text-gray-100 p-2 rounded hover:bg-[#f3929c] " onClick={() => navigate("/cart")}>Checkout Cart</button>
          </div>
          </div>
        </div>
      ) : (
