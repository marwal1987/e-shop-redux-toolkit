  ...
  return (
    <div className="w-full p-8 gap-12 rounded-sm shadow-md flex flex-col items-center justify-between border-b-2 border-gray-300">
      <Link to={`/product/${product.id}`} className="w-full flex flex-col items-center gap-6">
        ...
        <h2 className="text-lg font-bold text-center drop-shadow">{product.title}</h2>
      
      </Link>
      <div className="w-full flex items-end justify-between">
        <p className="text-gray-900 font-bold text-center text-xl font-serif drop-shadow">${product.price.toFixed(2)}</p>
      <button
